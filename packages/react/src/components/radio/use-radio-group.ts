...
export function useRadioGroup(props: UseRadioGroupProps = {}) {
    ...
    const handleChange = useCallback(
        (eventOrValue: React.ChangeEvent<HTMLInputElement> | string | number) => {
            const selectedValue: string | number = isInputEvent(eventOrValue)
                ? eventOrValue.target.value
                : eventOrValue;

            // Keep value type consistent - don't auto-convert strings to numbers
            // This ensures radio values match group values properly
