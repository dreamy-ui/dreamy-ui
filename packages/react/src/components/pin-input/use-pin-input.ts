...
export type PinInputContext = Omit<
    UsePinInputReturn<Record<string, any>, Record<string, any>>,
    "descendants"
> & {
    /**
...
export type PinInputPropsWithRef<T extends Record<string, any>> = T & {
    ref: React.Ref<HTMLInputElement>;
    index: number;
};

interface UsePinInputReturn<I extends Record<string, any>, W extends Record<string, any>> {
    getInputProps: (props: PinInputPropsWithRef<I>) => Record<string, any>;
    getWrapperProps: (props: W) => W;
    id: string;
 ...
 * @internal
 */
export function usePinInput<I extends Record<string, any>, W extends Record<string, any>>(
    props: UsePinInputProps = {}
): UsePinInputReturn<I, W> {
    const {
        autoFocus,
        value,
        defaultValue,
    ...
    const getInputProps = useCallback(
        (props: PinInputPropsWithRef<I>) => {
            ...
            const onPaste = (event: React.ClipboardEvent<HTMLInputElement>) => {
                ...
                if (validate(pastedData, type)) {
                    // Ensure the value matches the number of inputs
                    const nextValue = pastedData
                        .split("")
                        .filter((_, i) => i < descendants.count());

                    setValues(nextValue);

                    // autofocus on the last filled input or the last input if all are filled
                    requestAnimationFrame(() => {
                        const focusIndex = Math.min(nextValue.length - 1, descendants.count() - 1);
                        const inputToFocus = descendants.item(focusIndex);
                        if (inputToFocus) {
                            inputToFocus.node?.focus();
                        }
                    });
            ...
            return {
                ...
                onKeyDown: callAllHandlers(
                    inputRest.onKeyDown as (event: React.KeyboardEvent) => void,
                    onKeyDown
                ),
                onPaste: callAllHandlers(
                    inputRest.onPaste as (event: React.ClipboardEvent<HTMLInputElement>) => void,
                    onPaste
                ),
                onFocus: callAllHandlers(inputRest.onFocus as () => void, onFocus),
                onBlur: callAllHandlers(inputRest.onBlur as () => void, onBlur),
                value: values[index] || "",
    ...
    const getWrapperProps = useCallback((props: W) => {
        return {
            ...props
        };
    }, []);
...
export type UsePinInputFieldProps<I extends Record<string, any>> = I & {
    ref?: React.Ref<HTMLInputElement>;
};
