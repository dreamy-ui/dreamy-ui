...
import { addPointerEvent, getEventPoint, isMultiTouchEvent } from "@/utils/event";
...
export function usePanEvent(ref: React.RefObject<HTMLElement | null>, options: UsePanEventProps) {
	...
	useLayoutEffect(() => {
		...
		function onPointerDown(event: AnyPointerEvent) {
			panSession.current = new PanEvent(event, handlersRef, threshold);
		}
...
export class PanEvent {
	...
	private handlersRef: React.RefObject<Partial<PanEventHandlers>>;
	...
	constructor(
		...
		handlersRef: React.RefObject<Partial<PanEventHandlers>>,
		threshold?: number
	) {
		...
		const { onSessionStart } = this.handlersRef.current;
	...
	private get handlers() {
		return this.handlersRef.current;
	}
