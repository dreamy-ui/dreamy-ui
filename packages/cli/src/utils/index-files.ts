...
export async function writeRecipesIndexFile(recipesDir: string, jsx: boolean, debug: Debugger) {
    ...
    try {
        ...
        recipeFiles = allFiles.filter(
            (file) => (jsx && file.endsWith(".js")) || (!jsx && file.endsWith(".ts"))
        );
    } catch (error) {
    ...
    for (const file of recipeFiles) {
        try {
            ...
            if (recipeExport) {
                ...
                debug(`No recipe export found for ${expectedExportName} in ${file}`);
            }
        } catch (error) {
    ...
    const exports = recipeExports.map(({ exportName }) => exportName).join(",\n    ");
...
export async function writePatternsIndexFile(patternsDir: string, jsx: boolean, debug: Debugger) {
    ...
    for (const file of patternFiles) {
        try {
            ...
            if (patternExport) {
                ...
                debug(`No pattern export found for ${expectedExportName} in ${file}`);
            }
        } catch (error) {
            // Skip files that can't be read
    ...
    const exports = patternExports.map(({ exportName }) => exportName).join(",\n    ");
