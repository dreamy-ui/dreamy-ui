...
import { findCompositionById, getFileDependencies } from "../utils/get-file-dependencies";
import { ensureDir } from "../utils/io";
import { type CompositionFile, type Compositions, addCommandFlagsSchema } from "../utils/schema";
...
export const ComponentsCommand = new Command("components")
    ...
    .addCommand(
        new Command("list").description("List available components").action(async () => {
...
function getComponents(opts: {
    ...
    if (components.length === 0 && !all)
        return {
            message: "No component(s) selected, adding recommended components...",
