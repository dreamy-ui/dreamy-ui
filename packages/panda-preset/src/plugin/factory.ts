...
const factoryFileNewContent = `import { Children, createElement, forwardRef, useMemo } from 'react';
...
function styledFn(Dynamic, configOrCva = {}, options = {}) {
  ...
  const DreamyComponent = /* @__PURE__ */ forwardRef(function DreamyComponent(props, ref) {
    ...
    const combinedProps = useMemo(() => Object.assign({}, defaultProps, restProps), objectToDeps(restProps))
    ...
    if (asChild) {
      return createElement((Children.toArray(children)[0]?.type || Element), {
        ref: Element?.ref ? composeRefs(ref, Element?.ref) : ref,
        ...forwardedProps,
        ...elementProps,
        ...normalizeHTMLProps(htmlProps),
        ...(Children.toArray(children)[0]?.props || {}),
        className: classes()
