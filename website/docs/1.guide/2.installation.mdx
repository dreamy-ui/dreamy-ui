...
- Update `vite.config.ts` or `postcss.config.mjs` with Panda CSS PostCSS plugin
- Remove default Tailwind CSS (React Router v7 only)
- Set up CSS files with proper imports
- Create a `DreamyProvider` component
- Update your `tsconfig.json` with `@/*` path alias and styled-system (if using TypeScript)
  - Next.js also gets `styled-system/*` path alias for proper type resolution
