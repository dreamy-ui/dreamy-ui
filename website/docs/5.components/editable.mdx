...
Use `variant` to change how editability is signaled. `plain` (default) is text-only. `soft` rounds the preview with `l1` and shows an alpha background on hover and focus.

<Wrapper>
	<HStack alignItems="flex-start" gap={8}>
		<Editable.Root defaultValue="Plain" placeholder="Enter text" variant="plain">
			<Editable.Preview />
			<Editable.Input />
		</Editable.Root>
		<Editable.Root defaultValue="Soft" placeholder="Enter text" variant="soft">
			<Editable.Preview />
			<Editable.Input />
		</Editable.Root>
	</HStack>
