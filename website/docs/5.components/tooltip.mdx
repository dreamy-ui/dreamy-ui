...
Tooltip uses the shared [Portal](/docs/components/portal) stack at the `tooltip` layer (1800). Inside a Dreamy UI Modal, Popover, or other overlay, it becomes a nested scope and adds that layer to its parent base automatically. Keep the portal enabled—no manual layer or z-index workaround is needed.

Use `portalProps.containerRef` only to target the non-scrolling root of a native or third-party overlay.

<Wrapper>
	<Tooltip content="Automatically stacked tooltip">
		<Text w="fit-content">Hover me</Text>
	</Tooltip>
</Wrapper>
