...
Use `popoverProps` on `Menu.Root` to pass behavior props to the internal [Popover](/docs/components/popover). Keep placement options on the root `positioning` prop.

Menu uses the shared Portal stack at the `dropdown` layer (1000). Inside a Dreamy UI overlay, its scope nests automatically and adds 1000 to the parent base; nested menus continue the same scoped behavior. Keep the portal enabled—no manual layer or z-index is needed. For a native or third-party overlay, target its non-scrolling root with `popoverProps={{ portalProps: { containerRef } }}`.

<Wrapper>
    <Menu.Root popoverProps={{ lazyBehavior: "unmount" }}>
        <Menu.Trigger>
            <Button>Open Menu</Button>
        </Menu.Trigger>
        <Menu.Content>
...
Menu comes with 6 different sizes.
