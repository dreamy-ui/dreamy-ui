...
```bash	
pnpm dreamy components add action-bar
```
...
function ControlledActionBar() {
  ...
  return (
    <>
      ...
      <ActionBar.Root isOpen={isOpen} onClose={() => setIsOpen(false)}>
...
The simplest way to use the Action Bar is with controlled state.
...
function ActionBarSizes({ size }) {
  ...
  return (
    <>
      <Button onClick={() => setIsOpen(true)} size={size}>
        Show {size} Action Bar
      </Button>
      
      <ActionBar.Root isOpen={isOpen} onClose={() => setIsOpen(false)} size={size}>
        <ActionBar.Positioner>
          <ActionBar.Content>
            <ActionBar.SelectionTrigger>
              2 selected
