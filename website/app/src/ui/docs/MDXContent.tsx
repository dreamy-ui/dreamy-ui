...
export default function MDXContent({ mdxContent }: Props) {
	...
	return useMemo(() => {
		return (
			<MDXRemote {...mdxContent} components={{ ...components, toast }} />
		);
...
const components = {
	p: (props) => (
		<Text mb={2} color={"fg.medium"} fontWeight={400} {...props} />
	),
	h1: (props) => (
		<DefaultHeading as="h2" size={"3xl"} mt={10} mb={3} {...props} />
	),
	h2: (props) => (
		<DefaultHeading as="h3" size={"2xl"} mt={6} mb={2} {...props} />
	),
	h3: (props) => (
		<DefaultHeading
			as="h4"
		...
		if (href?.startsWith("/")) {
			return (
				...
				>
					{children}{" "}
					{isExternal && (
						<Icon as={HiExternalLink} display={"inline-block"} />
					)}
				</Link>
			);
		}
	...
	blockquote: (props) => {
		...
		if (typeof contentChildren === "string") {
			content = contentChildren.replace(
	...
	img: ({ alt = "image", ...props }) => {
		...
		return (
			...
			>
				<Image alt={alt} rounded={"md"} loading="lazy" {...props} />
				{alt && (
					<Text
						as={"span"}
						aria-hidden="true"
	...
	ul: (props) => <List unordered {...props} />,
	ol: (props) => <List ordered {...props} />,
	li: (props) => <ListItem {...props} />,
	strong: (props: any) => (
		<Text as="strong" color={"fg"} fontWeight={600} {...props} />
	),
	...DreamComponents,
	...icons
