...
function transformTree(parent: any) {
	...
	for (let i = 0; i < parent.children.length; i++) {
		...
		// Only consider code blocks with shell languages
		if (!node || node.type !== "code" || typeof node.value !== "string") continue;

		// Only transform code blocks with shell languages (bash, sh, shell, zsh, fish, etc.)
		const shellLanguages = ["bash", "sh", "shell", "zsh", "fish", "powershell", "ps1"];
		const codeLang = node.lang?.toLowerCase() || "";
		if (!shellLanguages.includes(codeLang)) continue;
