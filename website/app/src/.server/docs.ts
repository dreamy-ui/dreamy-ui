...
export class Docs {
				...
				const files = await Promise.all<DocsFile>(
					filenames.map(async (filename) => {
						...
						return cachified({
							...
							staleWhileRevalidate: Docs.shouldCacheDocs
								? daysToMs(30)
								: 0,
							getFreshValue: () => doc
						});
	...
	private static async fetchLocalDoc(filepath: string) {
		...
		const mdxContent = await Docs.serialize(content);
		Logger.debug(
			`Docs.serialize took ${performance.now() - serializeStart}ms`
		);
	...
	private static async fetchGithubDoc(filePath: string) {
		...
		if (!env.VITE_SOURCE_REPO) {
			throw new Error(
				"Missing required environment variable SOURCE_REPO"
			);
		}
	...
	private static async fetchGithubDocsFolder() {
		...
		const folderArr = (gh.data as any).filter(
		...
		for (let folder of folders) {
			...
			const files = (filenamesRes.data as any).filter(
			...
			docs.push({
				...
				sections: filenames.map((filename) => ({
					...
					slug: `/docs/${folder}/${filenameToSlug(
	...
	public static async fetchGithubDocsOnStartup() {
		...
		const folders = (gh.data as any).filter(
			(item: any) => item.type === "dir"
		);
		...
		for (const folder of folders) {
			...
			await Promise.all<DocsFile>(
				(files.data as any).map(async (file: any) => {
					...
					if (!downloadUrl) {
						throw new Error(
							"Missing download_url in response from GitHub"
						);
					}

					const content = await fetch(downloadUrl).then((res) =>
						res.text()
					);
		...
		return cachified({
			...
			getFreshValue: async () => {
				...
				const folderIndex = sections.find(
					(section) =>
						removeIndex(section.title).toLowerCase() ===
						folder.toLowerCase()
				)?.index;
				if (!folderIndex) {
				...
				const fileIndex = sections[folderIndex - 1].sections.find(
					(section) =>
						...
						filenameToTitle(page).toLowerCase()
				)?.index;
...
export interface MdxContent
	extends MDXRemoteSerializeResult<
